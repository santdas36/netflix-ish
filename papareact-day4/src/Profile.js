...
{packs.map((pack) => (
	<div className={pack.priceId === activePack ? 'pack active' : 'pack'}>
		...
		<button disabled={pack.priceId === activePack} onClick={(e)=> checkout(pack.priceId)}>{pack.priceId === activePack ? `Renews in ${parseInt((subs.current_period_end - subs.current_period_end)/(24*60*60))}` : 'Activate Now'}</button>
	</div>
))}
</div>
